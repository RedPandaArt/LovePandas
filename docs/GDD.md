...
#### Формат боя — ✅ решено: пошаговый кооп-бой (как Clair Obscur: Expedition 33)
Оба нажимают «В бой» и подтверждают → появляется босс → персонажи пары стоят вдвоём против него →
**пошагово** выбирают действия кнопками: «Царапки», «Громкий рык», «Бросок пыли» и т.п.

**Как проходит бой (предложение):**
1. **Лобби.** Один зовёт («Панда зовёт тебя на босса! 🐉» — push), второй подтверждает. Нужны оба онлайн.
2. **Фаза планирования.** Оба **одновременно** выбирают действие (таймер ~20–30 с) —
   никто не ждёт другого; видно, что выбрал партнёр, можно договориться.
3. **Фаза действий.** Персонажи бьют по очереди с анимацией.
4. **Ход босса.** Босс атакует; его **следующее намерение показано заранее**
   («Дракон готовит огненный выдох по обоим!») — чтобы пара планировала вместе.
5. Повтор, пока кто-то не победит.

**Ресурсы:** здоровье (HP) и **очки действий (ОД)**, как в E33: базовая атака даёт +1 ОД,
сильные умения тратят ОД.

**Умения (пример для красной панды):**
| Умение | ОД | Эффект |
|---|---|---|
| Царапки | 0 (+1 ОД) | Базовый урон |
| Громкий рык | 2 | Босс атакует тебя, а не партнёра (защита половинки) / ослабляет босса |
| Бросок пыли | 2 | Босс ослеплён — шанс промаха |
| Обнимашка | 1 | Лечит партнёра |

У каждого вида зверька свой набор (енот — ловкие трюки, котёнок — магия и т.д.).
Новые умения открываются уровнями карты прогресса.

**Парные механики — фишка игры:**
- **Комбо:** определённые пары умений в одном ходу дают совместную атаку
  (Рык + Бросок пыли → «Пыльная буря»).
- **Шкала «Вместе»:** заполняется от слаженных ходов → суперудар вдвоём с особой анимацией.
- **Поднять партнёра:** если персонаж упал — вторая половинка может его поднять.

**Тайминг как в E33 (позже, по желанию):** в момент удара тапнуть вовремя → бонус урона;
на атаку босса тапнуть вовремя → уклонение/парирование. Делает бой живым, но бой
проходится и без этого.

**Сила персонажей → бой:** сила/уровень пары даёт HP, урон и открытые умения.

**Если партнёр вылетел:** бой на паузе; через таймаут его персонаж делает базовую атаку автоматически.

**Техника:** пошаговый бой не требует реалтайм-мультиплеера. Состояние боя — в Firebase
(Realtime Database/Firestore). Оба клиента отправляют выбранные действия, результат считается
**детерминированно** (общий seed случайности) — у обоих одинаковый исход.
Альтернатива при необходимости: Photon / Nakama.

#### Боссы — олицетворение проблем пары
Боссы с юмором символизируют то, с чем пара реально борется:
Дракон Рутины, Ленивец Прокрастинации, Гидра Немытой Посуды, Туча Скуки,
