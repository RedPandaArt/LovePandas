...
**Платформа:** Android. **Движок:** Unity. **Графика:** 3D, мультяшная, милая.

**Главный принцип:** приложение должно быть **супер простым**.

**Контент первой версии:** см. [CONTENT_MVP.md](CONTENT_MVP.md) — красная панда ♂/♀, 1 закольцованный босс, упор на предметы кастомизации.
...
### 5.5. Сила персонажей и битвы с боссами (идея)
Прокачка — не просто цифра: **сила персонажей растёт**, и пара вместе идёт **сражаться с боссом**.
Победа → золото и плюшки → тратим, ставим задания, работаем над отношениями → становимся сильнее → следующий босс.

```
Задания друг другу ──► сила растёт ──► босс ──► победа: золото + лут
        ▲                                              │
        └──────────── тратим, ставим новые задания ◄───┘
```

Референс: в Habitica тоже есть боссы для группы — урон боссу наносят выполненные дела.

#### Откуда сила (предложение)
- Каждый выполненный квест/задание партнёра, мини-цели дня, уровни карты прогресса → **+сила**.
- У каждого персонажа своя сила (видно личный вклад), но с боссом бьётся **общая сила пары**.
- **Бонус слаженности:** если силы партнёров примерно равны — дополнительный множитель.
  Мотивирует обоих участвовать, но без «ты слабое звено» (показываем общий результат, не кто виноват).
- Позже: предметы с бонусами к силе (например, боевые наряды), классы по видам зверьков
  (панда — танк, енот — ловкач, котёнок — маг) — для колорита, не обязательно.

#### Формат боя — ✅ решено: простой пошаговый бой вдвоём
Референс по ощущению — пошаговые бои (Clair Obscur: Expedition 33), но **намного проще**.
Цель боя — не сложная тактика, а **ощущение «мы прошли это вместе и многое преодолели»**.

**Как проходит:**
1. Оба нажимают «В бой» и подтверждают (push партнёру: «Панда зовёт тебя на босса! 🐉»).
2. Появляется босс, персонажи пары стоят вдвоём против него.
3. По очереди: ход игрока 1 → ход игрока 2 → ход босса. На своём ходу — выбрать одну из
   **3–4 кнопок-умений**: «Царапки», «Громкий рык», «Бросок пыли», «Обнимашка» (лечит партнёра).
4. Полоска здоровья босса уменьшается, красивые анимации ударов.
5. Победа → финальный удар вдвоём (общая анимация) → экран «Вы победили Ленивца Прокрастинации вместе!» → награды.

**Принципы простоты:**
- Никаких очков действий, таймингов, сложных комбо и статов на экране.
- Сила пары просто влияет на урон и здоровье — чем больше делали друг для друга, тем быстрее победа.
- Бой короткий: **2–3 минуты**.
- Проиграть почти невозможно; если силы не хватило — «Босс отступил, возвращайтесь сильнее», без потерь.
- Победа сохраняется в **общую историю пары** («12 марта победили Ленивца Прокрастинации») + трофей в локации.

**Техника:** пошаговый бой не требует реалтайм-мультиплеера — ходы пишутся в Firebase,
оба клиента показывают одно и то же.

**Позже, если захочется:** уникальные умения у каждого вида зверька, совместная атака
при заполнении шкалы «Вместе», тап в такт для бонусного урона.

#### Боссы — олицетворение проблем пары
Боссы с юмором символизируют то, с чем пара реально борется:
✅ **Первый босс: Ленивец Прокрастинации** (подробно — в CONTENT_MVP.md). Идеи на будущее:
Дракон Рутины, Гидра Немытой Посуды, Туча Скуки,
Монстр Недосказанности, Кракен Телефона (вечно в телефоне) и т.п.
Боссы стоят **в конце каждой зоны карты прогресса** — победа открывает следующую зону/локацию.

#### Награды
- **Золото** — крупный приток монет от системы (помогает экономике, см. 7).
- **Эксклюзивный лут** с босса: наряды, которые нельзя купить в магазине.
- **Трофей** в локации/доме пары (голова дракона над камином 😄).
- Открытие следующей зоны карты.

#### Правила
- **Проигрыш без наказания:** монеты и вещи не теряются, просто повторить позже
  (после перезарядки или ещё немного прокачавшись).
- Бой — **только вдвоём**: нужны оба («Готов» от обоих). Кооператив, не соревнование.
- Золото с боссов учитывать в балансе, чтобы задания друг другу оставались главным заработком.
...
- **3D**, рендер-пайплайн **URP** (оптимально для мобильных).
- Стилизованная графика: **toon/cel-шейдер** или мягкий «глиняный» стиль, low/mid-poly.
- Бюджет для средних Android-телефонов: персонаж ~5–10 тыс. треугольников (с одеждой),
  1 материал/атлас текстур на персонажа, текстуры 512–1024 px, простое освещение (запечённое + 1 источник).
- Камера фиксированная: персонаж крупно по центру, можно слегка покрутить пальцем.
- UI — uGUI или UI Toolkit. Приложение на ~70% состоит из UI (доска, формы, магазин).
...
Проблема: N персонажей × M предметов = очень много моделей и подгонки.
Решение: **единый риг (скелет) у ♂/♀ и, по возможности, у будущих зверьков** + два типа предметов:
- **Жёсткие предметы** (шапки, очки, короны, рюкзаки, браслеты) — отдельная модель,
  крепится к кости/сокету (Head, Face, Neck, Back, Hand_L/R, Tail). Самые дешёвые в производстве —
  поэтому **основной объём контента делаем жёсткими предметами**.
- **Облегающие предметы** (футболки, шортики, пижама, перчатки) — skinned mesh на том же риге,
  привязанный к скелету персонажа (перепривязка костей при надевании).
  Дороже: нужна подгонка под тело, чтобы не было «прорезаний».
- Отличия ♂/♀ — детали головы/мордочки и, при необходимости, **blend shapes**, чтобы одежда
  садилась на обоих без отдельных моделей.
- Под одеждой, которая закрывает часть тела, можно скрывать куски тела (против прорезаний).
...
Item     { id, name, slot: Head|Body|Legs|Hands|..., price, prefab, icon, attachType: Rigid|Skinned }
